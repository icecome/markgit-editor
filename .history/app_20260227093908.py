...
def init_git():
    ...
    try:
        ...
        is_ssh = git_repo.startswith('git@') or git_repo.startswith('ssh://')
        ...
        if is_ssh:
            ...
            logger.info(f"Using HTTPS URL: {git_repo}")
        ...
        if 'Permission denied' in error_msg or 'password' in error_msg.lower():
            detail = "SSH authentication failed. Please check your SSH key configuration or use HTTPS URL with personal access token."
        elif 'Could not resolve host' in error_msg:
            detail = "Network error: Could not resolve host. Please check your internet connection."
        elif 'Repository not found' in error_msg or 'not found' in error_msg.lower():
            detail = "Repository not found. Please check the URL and your access permissions."
        else:
            detail = f"Failed to clone repository: {error_msg}"
        raise HTTPException(status_code=500, detail=detail)
    except subprocess.TimeoutExpired:
        logger.error("Git clone timed out")
        raise HTTPException(status_code=500, detail="Git clone timed out - check your network connection and SSH key configuration")
    except Exception as e:
