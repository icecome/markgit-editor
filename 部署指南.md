# MarkGit Editor 部署指南

**版本**: v1.2.0  
**更新日期**: 2026-02-28  

---

## 📋 目录

1. [部署概述](#部署概述)
2. [Docker 部署](#docker-部署)
3. [传统服务器部署](#传统服务器部署)
4. [PaaS 平台部署](#paas-平台部署)
5. [Serverless 部署](#serverless-部署)
6. [自托管 PaaS 部署](#自托管-paaS-部署)
7. [Kubernetes 部署](#kubernetes-部署)
8. [环境变量配置](#环境变量配置)
9. [常见问题](#常见问题)

---

## 部署概述

### 项目特点

| 特性 | 说明 | 部署影响 |
|------|------|----------|
| FastAPI 后端 | Python Web 框架 | 需要 Python 环境 |
| Git 命令行 | 执行 Git 操作 | 需要安装 Git |
| 文件系统存储 | 用户博客缓存 | 需要持久化存储 |
| OAuth 认证 | GitHub 登录 | 需要公网访问 |
| 会话管理 | 内存/Redis | 可选 Redis |

### 部署方案对比

| 方案 | 适用场景 | 难度 | 成本 | 持久化 | 推荐度 |
|------|----------|------|------|--------|--------|
| Docker | 任何平台 | ⭐⭐ | 中等 | ✅ | ⭐⭐⭐⭐⭐ |
| 传统服务器 | VPS、云服务器 | ⭐⭐⭐ | 中等 | ✅ | ⭐⭐⭐⭐ |
| Railway | 快速部署 | ⭐ | 低 | ✅ | ⭐⭐⭐⭐⭐ |
| Render | 免费套餐 | ⭐ | 低 | ✅ | ⭐⭐⭐⭐ |
| Fly.io | 边缘部署 | ⭐⭐ | 低 | ✅ | ⭐⭐⭐⭐ |
| Zeabur | 简单部署 | ⭐ | 低 | ✅ | ⭐⭐⭐⭐ |
| Koyeb | 全球部署 | ⭐⭐ | 中等 | ✅ | ⭐⭐⭐ |
| Coolify | 自托管 | ⭐⭐⭐ | 低 | ✅ | ⭐⭐⭐⭐ |
| Kubernetes | 大规模 | ⭐⭐⭐⭐ | 高 | ✅ | ⭐⭐⭐ |
| Cloudflare | 边缘 | ⭐⭐⭐⭐ | 低 | ❌ | ⭐⭐ |
| Vercel | Serverless | ⭐⭐⭐⭐ | 低 | ❌ | ⭐⭐ |

---

## Docker 部署

### 1. 创建 Dockerfile

```dockerfile
FROM python:3.11-slim

LABEL maintainer="MarkGit Editor"
LABEL version="1.2.0"

RUN apt-get update && apt-get install -y \
    git openssh-client curl \
    && rm -rf /var/lib/apt/lists/*

WORKDIR /app
COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

COPY . .
RUN mkdir -p /app/blog_cache

RUN useradd -m -u 1000 appuser && chown -R appuser:appuser /app
USER appuser

EXPOSE 13131
ENV PYTHONUNBUFFERED=1 PRODUCTION=true PORT=13131

HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
    CMD curl -f http://localhost:13131/ || exit 1

CMD ["uvicorn", "app:app", "--host", "0.0.0.0", "--port", "13131"]
```

### 2. Docker Compose

```yaml
version: '3.8'
services:
  markgit-editor:
    build: .
    ports: ["13131:13131"]
    environment:
      - PRODUCTION=true
      - GITHUB_CLIENT_ID=${GITHUB_CLIENT_ID}
      - GITHUB_CLIENT_SECRET=${GITHUB_CLIENT_SECRET}
      - REDIS_URL=redis://redis:6379/0
    volumes:
      - markgit-cache:/app/blog_cache
    depends_on: [redis]

  redis:
    image: redis:7-alpine
    volumes: [redis-data:/data]

volumes:
  markgit-cache:
  redis-data:
```

### 3. 部署命令

```bash
docker-compose up -d
docker-compose logs -f
```

---

## 传统服务器部署

### 快速部署脚本

```bash
# 下载并运行
curl -fsSL https://raw.githubusercontent.com/your-repo/markgit-editor/main/deploy/deploy.sh | bash -s -- your-domain.com
```

### 手动部署

详见 [deploy/deploy.sh](deploy/deploy.sh) 和 [deploy/nginx.conf](deploy/nginx.conf)。

---

## PaaS 平台部署

### Railway (推荐 ⭐⭐⭐⭐⭐)

**优势**: 一键部署、自动 SSL、持久化存储、内置 Redis

#### 1. 创建 `railway.toml`

```toml
[build]
builder = "DOCKERFILE"
dockerfilePath = "Dockerfile"

[deploy]
startCommand = "uvicorn main:app --host 0.0.0.0 --port $PORT"
healthcheckPath = "/"
healthcheckTimeout = 300
restartPolicyType = "ON_FAILURE"
restartPolicyMaxRetries = 10
```

#### 2. 部署步骤

```bash
# 安装 CLI
npm install -g @railway/cli

# 登录
railway login

# 初始化项目
railway init

# 添加 Redis
railway add --plugin redis

# 设置环境变量
railway variables set GITHUB_CLIENT_ID=your_id
railway variables set GITHUB_CLIENT_SECRET=your_secret
railway variables set CORS_ORIGINS=https://your-app.up.railway.app
railway variables set PRODUCTION=true

# 部署
railway up

# 打开应用
railway open
```

#### 3. 持久化存储

Railway 默认提供持久化存储，无需额外配置。

---

### Render (推荐 ⭐⭐⭐⭐)

**优势**: 免费套餐、自动 SSL、Docker 支持

#### 1. 创建 `render.yaml`

```yaml
services:
  - type: web
    name: markgit-editor
    env: docker
    dockerfilePath: ./Dockerfile
    envVars:
      - key: PRODUCTION
        value: true
      - key: GITHUB_CLIENT_ID
        sync: false
      - key: GITHUB_CLIENT_SECRET
        sync: false
      - key: REDIS_URL
        fromService:
          type: redis
          name: markgit-redis
          
  - type: redis
    name: markgit-redis
    plan: free
```

#### 2. 部署步骤

1. 连接 GitHub 仓库到 Render
2. 创建新的 Web Service
3. 选择 Docker 环境
4. 配置环境变量
5. 添加 Redis 服务

#### 3. 注意事项

- 免费套餐会在 15 分钟无活动后休眠
- 持久化存储需要付费套餐

---

### Fly.io (推荐 ⭐⭐⭐⭐)

**优势**: 全球边缘部署、持久化存储、免费额度

#### 1. 创建 `fly.toml`

```toml
app = "markgit-editor"
primary_region = "sin"

[build]
  dockerfile = "Dockerfile"

[env]
  PRODUCTION = "true"
  PORT = "13131"

[mounts]
  source = "markgit_data"
  destination = "/app/blog_cache"

[[services]]
  internal_port = 13131
  protocol = "tcp"

  [[services.ports]]
    handlers = ["http"]
    port = 80

  [[services.ports]]
    handlers = ["tls", "http"]
    port = 443

  [[services.tcp_checks]]
    interval = "30s"
    timeout = "5s"
    grace_period = "10s"
```

#### 2. 部署步骤

```bash
# 安装 CLI
curl -L https://fly.io/install.sh | sh

# 登录
fly auth login

# 创建应用
fly apps create markgit-editor

# 创建持久化存储
fly volumes create markgit_data --region sin --size 1

# 设置环境变量
fly secrets set GITHUB_CLIENT_ID=your_id
fly secrets set GITHUB_CLIENT_SECRET=your_secret
fly secrets set CORS_ORIGINS=https://markgit-editor.fly.dev

# 部署
fly deploy

# 打开应用
fly open
```

#### 3. 添加 Redis

```bash
# 创建 Redis
fly redis create

# 连接 Redis
fly redis attach <redis-id>
```

---

### Zeabur (推荐 ⭐⭐⭐⭐)

**优势**: 一键部署、中文界面、免费额度

#### 1. 部署步骤

1. 访问 [zeabur.com](https://zeabur.com)
2. 连接 GitHub 仓库
3. 选择项目并部署
4. 添加 Redis 服务
5. 配置环境变量

#### 2. 配置文件

Zeabur 自动检测 Dockerfile，无需额外配置。

---

### Koyeb (推荐 ⭐⭐⭐)

**优势**: 全球部署、自动扩展

#### 1. 部署步骤

```bash
# 安装 CLI
go install github.com/koyeb/koyeb-cli/cmd/koyeb@latest

# 登录
koyeb login

# 创建应用
koyeb app create markgit-editor

# 部署服务
koyeb service create markgit-editor \
  --app markgit-editor \
  --docker docker.io/your-username/markgit-editor \
  --ports 13131:http \
  --routes /:13131

# 设置环境变量
koyeb service update markgit-editor \
  --env GITHUB_CLIENT_ID=your_id \
  --env GITHUB_CLIENT_SECRET=your_secret
```

---

## Serverless 部署

### ⚠️ 限制说明

Serverless 平台有以下限制，**不适合直接部署**：

| 限制 | 说明 | 影响 |
|------|------|------|
| 无文件系统 | 函数是无状态的 | 无法持久化 blog_cache |
| 无 Git 命令 | 环境不支持 | 核心功能不可用 |
| 执行时间限制 | 10s-60s | Git 操作可能超时 |

### 替代方案：前端 + API 分离

```
Serverless (前端静态文件)
        ↓
    外部 API 服务器 (Docker/VPS)
        ↓
    GitHub API (Git 操作)
```

### Cloudflare Pages

```bash
# 部署前端静态文件
wrangler pages deploy ./static

# API 使用外部服务器
# 修改 main.js 中的 API_BASE
```

### Vercel

```bash
# 部署前端
vercel --prod

# API 使用外部服务器
# 配置 rewrites 指向 API
```

---

## 自托管 PaaS 部署

### Coolify (推荐 ⭐⭐⭐⭐)

**优势**: 开源免费、界面友好、支持 Docker Compose

#### 1. 安装 Coolify

```bash
curl -fsSL https://cdn.coollabs.io/coolify/install.sh | bash
```

#### 2. 部署应用

1. 访问 Coolify 控制面板
2. 创建新项目
3. 选择 Docker Compose 部署
4. 上传 `docker-compose.yml`
5. 配置环境变量
6. 部署

---

### CapRover

**优势**: 开源免费、简单易用

#### 1. 安装 CapRover

```bash
docker run -p 80:80 -p 443:443 -p 3000:3000 -v /var/run/docker.sock:/var/run/docker.sock -v /captain:/captain caprover/caprover
```

#### 2. 创建 `captain-definition`

```json
{
  "schemaVersion": 2,
  "dockerfilePath": "./Dockerfile"
}
```

#### 3. 部署

```bash
# 安装 CLI
npm install -g caprover

# 登录
caprover login

# 部署
caprover deploy
```

---

## Kubernetes 部署

### 1. 创建 Deployment

```yaml
# k8s/deployment.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: markgit-editor
spec:
  replicas: 2
  selector:
    matchLabels:
      app: markgit-editor
  template:
    metadata:
      labels:
        app: markgit-editor
    spec:
      containers:
      - name: markgit-editor
        image: your-registry/markgit-editor:latest
        ports:
        - containerPort: 13131
        env:
        - name: PRODUCTION
          value: "true"
        - name: REDIS_URL
          valueFrom:
            secretKeyRef:
              name: markgit-secrets
              key: redis-url
        - name: GITHUB_CLIENT_ID
          valueFrom:
            secretKeyRef:
              name: markgit-secrets
              key: github-client-id
        - name: GITHUB_CLIENT_SECRET
          valueFrom:
            secretKeyRef:
              name: markgit-secrets
              key: github-client-secret
        volumeMounts:
        - name: blog-cache
          mountPath: /app/blog_cache
        resources:
          requests:
            memory: "256Mi"
            cpu: "250m"
          limits:
            memory: "512Mi"
            cpu: "500m"
      volumes:
      - name: blog-cache
        persistentVolumeClaim:
          claimName: markgit-pvc
```

### 2. 创建 Service

```yaml
# k8s/service.yaml
apiVersion: v1
kind: Service
metadata:
  name: markgit-editor
spec:
  selector:
    app: markgit-editor
  ports:
  - port: 80
    targetPort: 13131
  type: ClusterIP
```

### 3. 创建 Ingress

```yaml
# k8s/ingress.yaml
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
  name: markgit-editor
  annotations:
    cert-manager.io/cluster-issuer: letsencrypt-prod
spec:
  tls:
  - hosts:
    - your-domain.com
    secretName: markgit-tls
  rules:
  - host: your-domain.com
    http:
      paths:
      - path: /
        pathType: Prefix
        backend:
          service:
            name: markgit-editor
            port:
              number: 80
```

### 4. 部署命令

```bash
kubectl apply -f k8s/
```

---

## 环境变量配置

### 必需变量

| 变量名 | 说明 | 示例 |
|--------|------|------|
| `GITHUB_CLIENT_ID` | GitHub OAuth 客户端 ID | `Ov23li...` |
| `GITHUB_CLIENT_SECRET` | GitHub OAuth 客户端密钥 | `eb848b...` |

### 可选变量

| 变量名 | 默认值 | 说明 |
|--------|--------|------|
| `PRODUCTION` | `false` | 生产环境标志 |
| `PORT` | `13131` | 服务端口 |
| `CORS_ORIGINS` | `http://localhost:13131` | 允许的来源 |
| `REDIS_URL` | - | Redis 连接地址 |
| `BLOG_CACHE_PATH` | `./blog_cache` | 缓存目录 |
| `SESSION_TIMEOUT_HOURS` | `1` | 会话超时时间 |
| `MAX_CONCURRENT_SESSIONS` | `100` | 最大并发会话数 |

---

## 常见问题

### Q1: Git 操作失败

**解决方案**:
1. 确保用户已通过 OAuth 登录
2. 检查 OAuth token 是否有效
3. 确保仓库权限正确

### Q2: 会话丢失

**解决方案**:
1. 配置 Redis 存储
2. 检查 `SESSION_TIMEOUT_HOURS` 设置

### Q3: CORS 错误

**解决方案**:
1. 检查 `CORS_ORIGINS` 配置
2. 确保包含前端域名

---

## 部署检查清单

### 部署前

- [ ] 配置 GitHub OAuth 应用
- [ ] 设置环境变量
- [ ] 准备 SSL 证书
- [ ] 配置域名解析

### 部署后

- [ ] 测试 OAuth 登录
- [ ] 测试 Git 操作
- [ ] 检查日志输出
- [ ] 配置监控告警

---

**文档版本**: v2.0  
**最后更新**: 2026-02-28
